...
Perl6::Parser::Pure - Extract a Perl 6 AST using a Perl 6 grammar.
...
=begin SYNOPSIS
    ...
    #
    my $tree = $ppp.to-tree( $source );
    say $ppp.dump-tree( $tree );

    # Return only the displayed tokens (including whitespace) in the document
    #
    my @token = $ppp.to-tokens-only( $source );

    # Return all tokens and structures in the document
    #
    my @everything = $ppp.to-list( $source );

=end SYNOPSIS

=begin DESCRIPTION

Uses a proper Perl 6 grammar brute-forced from the rakudo sources.
...
class Perl6::Parser::Pure:ver<0.3.0>
  {
  also does Debugging;
  also does Testing;
  ...
  method parse( Str $source )
    ...
    my $a = Perl6::Parser::Pure::Actions.new;
